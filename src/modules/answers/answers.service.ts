...
export class AnswersService {
  ...
  async addAnswerToQuestion(answerDto: AnswerDto, user: UsersEntity) {
    ...
    try {
      const questionToUpdate = await this.dataSource.manager.findOne(
        QuestionsEntity,
        { where: { uuid: answerDto.questionUuid } },
      );
  ...
  async deleteAnswer(uuid: string) {
    try {
      ...
      if (!answer)
        throw new HttpException("Answer doesn't exists", HttpStatus.NOT_FOUND);
  ...
  async updateAnswer(answerDto: UpdateAnswerDto) {
    ...
    await queryRunner.startTransaction();
    try {
      const answerToEdit = await queryRunner.manager.findOne(AnswersEntity, {
        where: { uuid: answerDto.uuid },
      });
      if (!answerToEdit)
        throw new HttpException("Answer doesn't exists", HttpStatus.NOT_FOUND);
  ...
  async getAllAnswers() {
    ...
    await queryRunner.startTransaction();
    try {
      const answers = await this.dataSource.manager.find(AnswersEntity, {
